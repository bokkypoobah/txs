const Assets = {
  template: `
    <div class="m-0 p-0">
      <b-card no-body no-header class="border-0">
        ...
        <!-- TAGBAR -->
        <div v-if="false && settings.taggingMode" class="d-flex flex-wrap m-0 mt-1 p-0">
          <div class="mt-0 pr-1" style="max-width: 8.0rem;">
  ...
  methods: {
    ...
    async timeoutCallback() {
      logDebug("Assets", "timeoutCallback() count: " + this.count);
      this.count++;
      var t = this;
      if (this.reschedule) {
        setTimeout(function() {
  ...
  mounted() {
    ...
    logDebug("Assets", "Calling timeoutCallback()");
...
const assetsModule = {
  ...
  actions: {
    async restoreState(context) {
      const CHAIN_ID = 1;
      logInfo("assetsModule", "restoreState()");
    ...
    async saveData(context, types) {
      const CHAIN_ID = 1;
      logInfo("assetsModule", "actions.saveData - types: " + JSON.stringify(types));
      const db = store.getters['data/db'];
      const db0 = new Dexie(db.name);
      db0.version(db.version).stores(db.schemaDefinition);
    ...
    async generateReport(context, contractOrTxOrBlockRange) {
      logInfo("assetsModule", "generateReport(): " + (contractOrTxOrBlockRange || '(no parameters)'));
